% lines 6 & 13 must be changed for files. lines 17~23 can be changed with
...
% log file
logFilePath = "data/logfiles_04_24_23/archive/20230424_092421.log"; % change here to your log file path
...
recordFilePath = "data/watchrecords_04_24_23/watchRecord_m23_2023-4-24_part1.txt"; % change here to your record file path
...
for i=1:height(watchRecord)
    ...
    sT = watchRecord(i, 13:19);
    sT = sT{1,:};
    sT_padded = sT;
    eT = watchRecord(i, 20:26);
    eT = eT{1,:};
    eT_padded = eT;

    storagePath = "truncated_files/" + userID + "/" + vidNameNoExtension{1};
    ...
    if (startRow + 1 < endRow)
        ...
        trialPath = 'truncated_files/'+userID+'/trials.txt';
        if (isfile(trialPath))
        ...
        writetable(trials, 'truncated_files/'+userID+'/trials.txt');
        ...
        info_line = vidNameNoExtension(1) + "," + trialNumber + "," + (watchRecord{i, 3}) + "," + (watchRecord{i, 4}) + "," + (watchRecord{i, 5}) + "," + (watchRecord{i, 6}) + "," + (watchRecord{i, 7}) + "," + (watchRecord{i, 8}) + "," + (watchRecord{i, 9}) + "," + (watchRecord{i, 10}) + "," + (watchRecord{i, 11}) + "," + (watchRecord{i, 12}) + "\n";
