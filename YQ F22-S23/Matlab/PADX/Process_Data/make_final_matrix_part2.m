...
userid = "m2300"; % USERS: ONLY EDIT THIS LINE
part = "2";
...
pupil_index = 14; % col of pupil diameter in table. later make it automated
old_names=[];
new_names=[];
trial_numbers=[];
...
for i=1:height(trials_table)
    ...
    for t=1:trials_count
        ...
        rating_numbers10 = [rating_numbers10 info_table{t,12}];
...
for i=1:matrix_width
    ...
    for r=1:height(data_p)
        if (data_p{r, 26} == 1 && data_p{r, 27} == 1 && data_p{r, 28} == 1) % quality data
            matrix(r+num_info_rows, i) = data_p{r, pupil_index};
        end
    end
...
writetable(matTable, "final_matrices/" + string(userid) + "/" + string(userid) + "_part"+part+"_final");
