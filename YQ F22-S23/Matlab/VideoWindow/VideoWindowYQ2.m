%  lines to change: 
    % line 44: dimension of the video, in the format [width, height]
    % line 52: read the comments above it
    % line 66: make the same changes to the 'position' array as line 52

classdef VideoWindowYQ2  < handle
    ...
    methods
        function obj=VideoWindowYQ2(file, hide, varargin)
            ...
            figPos=[550 320]; % currently 3 inches (430px) tall. change the righter number to 180 for 1 inch tall video.

            %Create Matlab Figure Container
            if hide
                % the array after 'position' contains [x-coordinate,
                % y-coordinate, maxwidth, maxheight]. since it requires the video to be in scale, the video frame will always be limited by maxheight (since the maxwidth I set (600) is pretty large)
                % (figPos is an array of 2 elements that can be changed on
                % line 44)
                obj.Fig=figure('MenuBar','none','position',[400 250 figPos],... 
                ...
                obj.Fig=figure('MenuBar','none','position',[530 330 475 1000],...
                    'WindowState', 'normal',...
                    'Color', [0.423 0.423 0.423],...
                    'NumberTitle','off','Name',obj.ActxOpt,...
                    'CloseRequestFcn',@(src,evts) delete(obj),...
                    'ResizeFcn',@(src,evts) resize(obj),...
                    'ButtonDownFcn',@(src,evts) keyDown_Callback(obj));
                try
                    %Create ActiveX Controller
                    obj.Actx=actxcontrol(obj.ActxName,'parent',obj.Fig,...
                        'position',[0 -60 475 320]);
                        %'position',[490 230 figPos]);
                catch %#ok<CTCH>
                    error('Error during ActiveX openning.')
                end
            end
