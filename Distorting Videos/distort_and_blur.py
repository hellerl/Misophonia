...
# block number is the n you want to choose for a n x n splicing of a video
BLOCK_NUM = 16

# Set the Gaussian kernel size and standard deviation
# kernal size, in charge of blurring the video, higher means blurrier
# note that the numbers can only be odd, can't be even
K_SIZE = (11,11)
# also in charge of blurring the video, higher means blurrier
SIGMA = 3
...
indices_array = set_randomization(BLOCK_NUM)
...
def image_manipulation(input_folder): 
    ...
    for filename in os.listdir(input_folder):
        ...
        if filename.endswith(".jpg") or filename.endswith(".jpeg") or filename.endswith(".png"):
            ...
            img = shuffle_image(img,BLOCK_NUM)

            # Apply Gaussian blur using "reflect" border mode
            img = cv2.GaussianBlur(img, K_SIZE, SIGMA, borderType=cv2.BORDER_REFLECT)
